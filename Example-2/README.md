...
```bash
docker run -d --name "nginx-demo" -p 8080:80 -v $(pwd)/html/:/usr/share/nginx/html:ro nginx:latest
```
...
We can try to change **$(pwd)/html** (which is mounted as a volume to **/usr/share/nginx/html** directory inside the container) to **$(pwd)/sample-index** and refresh the page.
